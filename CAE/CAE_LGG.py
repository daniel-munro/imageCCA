...
if os.path.isfile('CAE_LGG/pretrained.h5'):
	cae.load_weights('CAE_LGG/pretrained.h5')

# train the model
for i in xrange(startEpoch, startEpoch + numEpochs):
	imBatch = getBatch(batchSize, 'LGG_images/') / 255.
...
#	cae.save_weights('pretrained.h5',overwrite=True)
	...
	if (i + 1) % 50 == 0:
		cae.save_weights('CAE_LGG/pretrained_' + str(i) + '.h5')
